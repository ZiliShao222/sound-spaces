...
@register_lifelong_eval_policy("no_audio_omega_nav")
@register_lifelong_eval_policy("no_audio_omega_nav_policy")
@register_lifelong_eval_policy("no_audio_omega_oracle")
@register_lifelong_eval_policy("no_audio_omega_nav_oracle")
class NoAudioOmegaNavPolicy(LifelongEvalPolicy):
    def __init__(
        self,
    ...
    def _resolve_episode_audio_labels( # 此函数在no_audio下其实没有被调用
        self,
        episode: Any
    ) -> Tuple[str, ...]:
    ...
    def _match_label(
        ...
        if not text:
            return None
        for alias, label in self._perception.audio_runtime.alias_to_category.items():
            token = normalize_text(alias).lower().replace("_", " ")
            if token and token in text and label in self._audio_global:
                return str(label)
        label = self._perception.audio_runtime.resolve_category(text)

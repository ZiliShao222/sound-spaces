...
class WorkerEnv:
    def __init__(self, env_fn, env_fn_arg, auto_reset_done):
        self._env = env_fn(*env_fn_arg)
...
class SyncVectorEnv:
    ...
    def __init__(
        ...
        # )
        self.workers = []
        for env_fn_arg in env_fn_args:
            worker = WorkerEnv(make_env_fn, env_fn_arg, auto_reset_done=auto_reset_done)
        ...
        # ]
        self.observation_spaces = [worker(OBSERVATION_SPACE_COMMAND, None) for worker in self.workers]
        self.action_spaces = [worker(ACTION_SPACE_COMMAND, None) for worker in self.workers]
        # for write_fn in self._connection_write_fns:
    ...
    def current_episodes(self):
        ...
        # self._is_waiting = False
        results = [worker(EPISODE_COMMAND, None) for worker in self.workers]
        return results
    ...
    def reset(self):
        ...
        #     results.append(read_fn())
        # self._is_waiting = False
        results = [worker(RESET_COMMAND, None) for worker in self.workers]
        return results
    ...
    def reset_at(self, index_env: int):
        ...
        # self._is_waiting = False
        results = [self.workers[index_env](RESET_COMMAND, None)]
        return results
    ...
    def step_at(self, index_env: int, action: Dict[str, Any]):
        ...
        # results = [self._connection_read_fns[index_env]()]
        # self._is_waiting = False
        results = [self.workers[index_env](STEP_COMMAND, action)]
        return results
    ...
    def step(self, data: List[Union[int, str, Dict[str, Any]]]) -> List[Any]:
        ...
        results = [worker(STEP_COMMAND, args) for worker, args in zip(self.workers, data)]
        return results
    ...
    def pause_at(self, index: int) -> None:
        ...
        self._paused.append((index, worker))
    ...
    def resume_all(self) -> None:
        ...
        # self._paused = []
        for index, worker in reversed(self._paused):
    ...
    def call_at(
        ...
        """
        # getattr(foo, 'bar')()
        result = getattr(self.workers[index]._env, function_name)(**function_args)
        # self._is_waiting = True

...
MODEL_DIR="${MODEL_DIR:-data/models/omni_long_mp3d_bc_no_image_text}"  #规定模型存放位置
EXP_CONFIG="${EXP_CONFIG:-ss_baselines/omni_long/config/omni_long/mp3d/bc_no_image_text_pointgoal_rgb.yaml}"  #规定配置文件位置
...
echo "============================================================"
echo "[omni-long-no-text-image] run_type=${RUN_TYPE}"
echo "[omni-long-no-text-image] model_dir=${MODEL_DIR}"
echo "[omni-long-no-text-image] exp_config=${EXP_CONFIG}"
echo "[omni-long-no-text-image] train_split=${TRAIN_SPLIT} eval_split=${EVAL_SPLIT}"
echo "[omni-long-no-text-image] goal_order_mode=${GOAL_ORDER_MODE}"
echo "[omni-long-no-text-image] num_processes=${NUM_PROCESSES} num_updates=${NUM_UPDATES}"
echo "[omni-long-no-text-image] torch_gpu_id=${TORCH_GPU_ID} simulator_gpu_id=${SIMULATOR_GPU_ID}"
echo "[omni-long-no-text-image] NOTE: This baseline uses RGB + Depth + Audio + Audio Spectrogram as goal, NO text and image goals"
echo "============================================================"

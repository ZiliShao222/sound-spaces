...
MODEL_DIR="${MODEL_DIR:-data/models/omni_long_mp3d_bc}"
EXP_CONFIG="${EXP_CONFIG:-ss_baselines/omni_long/config/omni_long/mp3d/bc_spectrogram_pointgoal_rgb.yaml}"
...
echo "============================================================"
echo "[omni-long-bc] run_type=${RUN_TYPE}"
echo "[omni-long-bc] model_dir=${MODEL_DIR}"
echo "[omni-long-bc] exp_config=${EXP_CONFIG}"
echo "[omni-long-bc] train_split=${TRAIN_SPLIT} eval_split=${EVAL_SPLIT}"
echo "[omni-long-bc] goal_order_mode=${GOAL_ORDER_MODE}"
echo "[omni-long-bc] num_processes=${NUM_PROCESSES} num_updates=${NUM_UPDATES}"
echo "[omni-long-bc] torch_gpu_id=${TORCH_GPU_ID} simulator_gpu_id=${SIMULATOR_GPU_ID}"
echo "============================================================"

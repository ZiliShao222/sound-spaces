...
def _episode_payload(
    ...
    return {
        ...
        "goal_order_mode": str(
            goal_order_mode
            or getattr(episode, "goal_order_mode", "")
            or ""
        ),
...
def parse_args() -> argparse.Namespace:
    ...
    args.submit_action_name = "LIFELONG_SUBMIT"
    args.video_dir = None
...
class OmniLongEnvServer:
    def __init__(self, args: argparse.Namespace):
        ...
        dataset = getattr(self.env, "_dataset", None) or getattr(self.env, "dataset", None)
        scene_instances = getattr(dataset, "instances_by_scene", None)
        self.instances_by_scene: Dict[str, Dict[str, Dict[str, Any]]] = {}
        if isinstance(scene_instances, dict):
            self.instances_by_scene = {
                str(scene_id): records
                for scene_id, records in scene_instances.items()
        ...
        self.current_video = bool(args.video)
        self.current_video_fps: Optional[int] = args.video_fps
        self.current_video_audio_normalize = bool(args.video_audio_normalize)
        self.current_video_audio_max_gain = float(args.video_audio_max_gain)
        self.current_video_frames: List[np.ndarray] = []
        self.current_video_audios: List[np.ndarray] = []
    ...
    def _scene_instance_records(self, scene_id: str) -> Dict[str, Dict[str, Any]]:
        key = str(scene_id)
        cached = self.instances_by_scene.get(key)
        if isinstance(cached, dict) and cached:
            return cached

        content_template = self.dataset_payload.get("content_scenes_path")
        if not isinstance(content_template, str) or "{scene}" not in content_template:
            return {}

        scene_name = _scene_key(key)
        data_path = str(Path(self.args.dataset_path).resolve().parent)
        content_path = content_template.replace("{data_path}", data_path).replace("{scene}", scene_name)
        payload = _load_dataset_payload(content_path)
        records = _flatten_instances(payload.get("instances", {}))
        self.instances_by_scene[key] = records
        return records
        ...
        for instance_key, modality in goal_specs:
            goal_inputs.append(
                _build_goal_input_payload(
                    ...
                    modality=modality,
                    instance_record=scene_records.get(instance_key),
                )
            )
    ...
    def _step_message(self, action: Any) -> Dict[str, Any]:
        ...
        step_result = self.env.step({"action": DEFAULT_ACTION_MAP[int(action)]})
        reward = None
        info: Dict[str, Any] = {}
        ...
        if self.current_video:
            ...
            self.current_video_frames.append(observations_to_image(observations, frame_info))
            if "audiogoal" not in observations:
                raise RuntimeError("video enabled but observation missing 'audiogoal'")
            self.current_video_audios.append(np.asarray(observations["audiogoal"], dtype=np.float32))
        ...
        if self.current_done:
            ...
            message["episode"] = _episode_payload(
                self.current_episode,
                self.current_goal_payloads,
                getattr(self.current_episode, "goal_order_mode", None) or self.args.goal_order_mode,
            )
            message["episode_metrics"] = episode_metrics
            message["summary"] = self._summary()
            if self.current_video and self.current_video_frames:
                video_root = os.path.join(self.run_base_dir, "videos")
                ...
                if fps is None:
                    ...
                    fps = max(1, int(fps))
                if len(self.current_video_audios) == 0:
                    raise RuntimeError("video enabled but no audio frames were collected")
                muxed_audios = list(self.current_video_audios)
                raw_peak = 0.0
                for clip in self.current_video_audios:
                    if np.asarray(clip).size > 0:
                        raw_peak = max(raw_peak, float(np.max(np.abs(np.asarray(clip, dtype=np.float32)))))
                if self.current_video_audio_normalize and raw_peak > 0.0:
                    gain = min(float(self.current_video_audio_max_gain), 0.8 / raw_peak)
                    if abs(gain - 1.0) > 1e-6:
                        muxed_audios = [
                            np.clip(np.asarray(clip, dtype=np.float32) * gain, -1.0, 1.0)
                            for clip in self.current_video_audios
                        ]
                video_name = f"episode_{int(self.current_episode_index)}"
                images_to_video_with_audio(
                    list(self.current_video_frames),
                    video_root,
                    video_name,
                    list(muxed_audios),
                    int(getattr(getattr(self.cfg.SIMULATOR, "AUDIO", None), "RIR_SAMPLING_RATE", 16000)),
                    fps=int(fps),
                )
                message["video_path"] = os.path.join(video_root, f"{video_name}.mp4")
            print(
                "[env_server][episode_done] "
                + json.dumps(
                    {
